...
class Barrier_Fncs():
    ...
    def torch_get_relative_positions_and_safety_functions(self, x):
        ...
        for j in range(nn):
            ...
            P[:, curr_idx, :] = x[:, idx] * (1 - self.param.r_agent * torch.pow(
                torch.norm(x[:, idx], p=2, dim=1).unsqueeze(1), -1))
            H[:, curr_idx] = (torch.norm(P[:, curr_idx, :], p=2, dim=1) -
                              self.param.r_agent)/(self.param.r_comm - self.param.r_agent)
            curr_idx += 1
        ...
        for j in range(no):
            ...
            H[:, curr_idx] = (torch.norm(closest_point, p=2, dim=1) -
                              self.param.r_agent)/(self.param.r_comm - self.param.r_agent)
            curr_idx += 1
    ...
    def torch_get_cf_si_2(self, x, pi, barrier_action, P, H):
        ...
        adaptive_scaling = torch.ones(
            (H.shape[0], 1), device=self.device) - epsilon
        # print('H',H)
        if not H.nelement() == 0:
            ...
            grad_phi = self.torch_get_grad_phi(x, P, H)
            A1 = self.param.kp * \
                torch.pow(torch.norm(grad_phi, p=2, dim=1), 2).unsqueeze(1)
    ...
    def torch_fdbk_di(self, x, P, H):
        ...
        b = -self.param.kv*(v + self.param.kp*grad_phi) - \
            self.param.kp*grad_phi_dot - self.param.kp*grad_phi
        return b
    ...
    def torch_get_cf_di_2(self, x, pi, b, P, H):
        ...
        if not H.nelement() == 0:
            ...
            A1 = self.param.kp**2 * torch.pow(torch.norm(grad_phi, p=2, dim=1, keepdim=True), 2) + \
                self.param.kv * \
                torch.pow(torch.norm(vmk, p=2, dim=1, keepdim=True), 2)
    ...
    def torch_get_grad_phi_dot(self, x, P, H):
        ...
        for j in range(self.get_num_neighbors(x) + self.get_num_obstacles(x)):
            ...
            f2dot = -torch.mul(torch.bmm(p_rel, v_rel).squeeze(2),
                               torch.pow(normp, -3))
            f3dot = -torch.mul(torch.bmm(p_rel, v_rel).squeeze(2), torch.mul(
                torch.pow(normp, -1), torch.pow(normp-self.param.r_agent, -2)))
    ...
    def numpy_get_relative_positions_and_safety_functions(self, x):
        ...
        for j in range(nn):
            idx = self.get_agent_pos_idx_j(x, j)
            P[:, curr_idx, :] = x[:, idx] * \
                (1 - self.param.r_agent / np.linalg.norm(x[:, idx]))
            H[:, curr_idx] = (np.linalg.norm(
                P[:, curr_idx, :]) - self.param.r_agent)/(self.param.r_obs_sense-self.param.r_agent)
            curr_idx += 1
        ...
        for j in range(no):
            ...
            P[:, curr_idx, :] = closest_point
            H[:, curr_idx] = (np.linalg.norm(
                closest_point) - self.param.r_agent)/(self.param.r_obs_sense-self.param.r_agent)
            curr_idx += 1
        return P, H
    ...
    def numpy_fdbk_di(self, x, P, H):
        ...
        b = -1*self.param.kv*(v + self.param.kp*grad_phi) - \
            self.param.kp*grad_phi_dot - self.param.kp*grad_phi
        return b
    ...
    def numpy_get_cf_di_2(self, x, P, H, pi, b):
        ...
        if not H.size == 0 and np.min(H) < self.param.Delta_R / (self.param.r_comm - self.param.r_agent):
            ...
            vmk = v + self.param.kp * grad_phi
            A1 = self.param.kp**2 * \
                np.dot(grad_phi, grad_phi.T) + \
                self.param.kv * np.dot(vmk, vmk.T)
            A2 = np.dot(vmk, (pi + self.param.kp*grad_phi_dot).T) + \
                self.param.kp * np.dot(grad_phi, v.T)
    ...
    def numpy_get_grad_phi_dot(self, x, P, H):
        ...
        for j in range(self.get_num_neighbors(x)+self.get_num_obstacles(x)):
            ...
            if normp > 0:
                ...
                f3dot = -1/(normp*(normp-self.param.r_agent)**2) * \
                    np.dot(p_rel, v_rel.T)
    ...
    def numpy_get_grad_phi(self, x, P, H):
        ...
        for j in range(self.get_num_neighbors(x) + self.get_num_obstacles(x)):
            grad_phi += self.numpy_get_grad_phi_contribution(
                P[:, j, :], H[:, j])
        return grad_phi
    ...
    def get_num_obstacles(self, x):
        nn = self.get_num_neighbors(x)
        # number of obstacles
        return int((x.shape[1] - 1 - self.dim_state - nn*self.dim_neighbor) / 2)
    ...
    def get_agent_vel_idx_j(self, x, j):
        idx = 2+1+self.dim_state + self.dim_neighbor * \
            j+np.arange(0, 2, dtype=int)
        return idx

    def get_obstacle_idx_j(self, x, j):
        nn = self.get_num_neighbors(x)
        idx = 1 + self.dim_state + self.dim_neighbor * \
            nn+j*2+np.arange(0, 2, dtype=int)
        return idx

    def get_agent_idx_all(self, x):
        nn = self.get_num_neighbors(x)
        idx = np.arange(1+self.dim_state, 1+self.dim_state +
                        self.dim_neighbor*nn, dtype=int)
        return idx

    def get_obstacle_idx_all(self, x):
        nn = self.get_num_neighbors(x)
        idx = np.arange((1+self.dim_state)+self.dim_neighbor *
                        nn, x.size()[1], dtype=int)
        return idx
