    ...
    def batch_loader(env, dataset, device):
        ...
        for data in dataset:
            ...
            if env.param.env_name in ['SingleIntegrator', 'DoubleIntegrator']:
                num_obstacles = int(
                    (data.shape[0] - 1 - env.state_dim_per_agent - num_neighbors*env.state_dim_per_agent - 2) / 2)
            elif env.param.env_name == 'SingleIntegratorVelSensing':
                num_obstacles = int(
                    (data.shape[0] - 1 - 2 - num_neighbors*4 - 2) / 2)
        ...
        for key, dataset_per_key in dataset_dict.items():
            ...
            print(name, " neighbors ", num_neighbors, " obstacles ",
                  num_obstacles, " ex. ", batch_x.shape[0])
...
def load_loader(name, batch_size, device, param):
    ...
    loader = []
    datadir = glob.glob(
        "../{}/batch_{}*.npy".format(param.preprocessed_data_dir, name))
    for file in datadir:
...
def test(param, env, model, loader):
    ...
    epoch_loss = 0
    for step, (b_x, b_y) in enumerate(loader):  # for each training step
        # input batch state and predict batch action
...
def train_il(param, env, device):
    ...
    if not param.il_load_loader_on:
        ...
        for datapattern, num_data in param.datadict.items():
            if param.env_name in ['SingleIntegrator']:
                datadir = glob.glob(
                    "../data/training/singleintegrator/central/*{}*.npy".format(datapattern))
            elif param.env_name in ['DoubleIntegrator']:
                datadir = glob.glob(
                    "../data/training/doubleintegrator/central/*{}*.npy".format(datapattern))
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_count()) as executor:
                for dataset in executor.map(env.load_dataset_action_loss, datadir):
                    ...
                    print('num_agents,len_case = {},{}'.format(
                        datapattern, len_case))
    ...
    # init model
    if param.il_controller_class == 'Barrier':
        model = Barrier_Net(param).to(device)
    elif param.il_controller_class == 'Empty':
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=param.il_lr, weight_decay=param.il_wd)
    ...
    with open(param.il_train_model_fn + ".csv", 'w') as log_file:
        ...
        for epoch in range(1, param.il_n_epoch+1):
            ...
            train_epoch_loss = train(
                param, env, model, optimizer, loader_train)
            ...
            if epoch % param.il_log_interval == 0:
                ...
                if test_epoch_loss < best_test_loss:
                    ...
                    model.to(device)
            log_file.write("{},{},{},{}\n".format(
                time.time() - start_time, epoch, train_epoch_loss, test_epoch_loss))
