...
class SingleIntegrator(Env):
    ...
    def observe(self):
        ...
        for agent_i in self.agents:
            ...
            obs_array = np.zeros(3+2*num_neighbors+2 *
                                 num_obstacles, dtype=np.float32)
    ...
    def reward(self):
        ...
        if len(results) > 0:
            for result in results:
                ...
                print('   a2a dist {} at time {} a {} and a {}'.format(
                    dist, self.times[self.time_step], self.agents[result[0]].i, self.agents[result[1]].i))
                # exit()
            return -1
        ...
        for agent in self.agents:
            for o in self.obstacles:
                coll, dist = not_batch_is_collision_circle_rectangle(np.array(
                    agent.p), self.param.r_agent, np.array(o), np.array(o) + np.array([1.0, 1.0]))
                inc = np.count_nonzero(coll)
                if inc > 0:
                    print('   a2o dist {} at time {} a {}'.format(
                        dist, self.times[self.time_step], agent.i))
                    return -inc
    ...
    def reset(self, initial_state=None):
        ...
        self.obstacles_np = np.array(
            [np.array(o) + np.array([0.5, 0.5]) for o in self.obstacles])
    ...
    def find_collision_free_state(self, config):
        ...
        while collision:
            ...
            s = self.init_state_mean + \
                self.init_state_var * \
                np.random.uniform(size=(self.state_dim_per_agent))
    ...
    def load_dataset_action_loss(self, filename):
        ...
        filename_map = "{}/../instances/{}.yaml".format(
            os.path.dirname(filename), instance)
        ...
        for x in range(-1, map_data["map"]["dimensions"][0]+1):
            ...
            obstacles.append(
                np.array([x, map_data["map"]["dimensions"][1]]) + np.array([0.5, 0.5]))
        for y in range(map_data["map"]["dimensions"][0]):
            obstacles.append(np.array([-1, y]) + np.array([0.5, 0.5]))
            obstacles.append(
                np.array([map_data["map"]["dimensions"][0], y]) + np.array([0.5, 0.5]))
        ...
        for t in range(0, data.shape[0]-1):
            ...
            # build kd-tree
            positions = np.array([data[t, i*4+1:i*4+3].numpy()
            ...
            for i in range(num_agents):
                ...
                s_g = torch.Tensor(
                    map_data["agents"][i]["goal"]) + torch.Tensor([0.5, 0.5])
                ...
                obs_array = np.empty(
                    3+2*num_neighbors+2*num_obstacles+2, dtype=np.float32)
                obs_array[0] = num_neighbors
                idx = 1
                obs_array[idx:idx+2] = relative_goal
                idx += 2
                ...
                transformed_oa_pairs, _ = self.preprocess_transformation(
                    [(obs_array[0:-2], obs_array[-2:])])
    ...
    def preprocess_transformation(self, dataset_batches):
        ...
        for (dataset, classification) in dataset_batches:
            ...
            transformed_classification = np.empty(
                classification.shape, dtype=np.float32)
            transformations = np.empty(
                (dataset.shape[0], 2, 2), dtype=np.float32)
            ...
            for k, row in enumerate(dataset):
                ...
                if classification is not None:
                    transformed_classification[k, :] = np.matmul(
                        R, classification[k])
                transformed_dataset[k, :] = transformed_row
                transformations[k, :, :] = R

            transformed_dataset_batches.append(
                (transformed_dataset, transformed_classification))
            ...
            for state in states:
                for i in range(self.n_agents):
                    ...
                    vis["agent" + str(i)].set_transform(
                        tf.translation_matrix([pos[0], pos[1], 0]))
                time.sleep(dt)
