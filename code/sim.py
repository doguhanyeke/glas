...
def sim(param, env, controllers, initial_state, visualize):
    ...
    for name, controller in controllers.items():
        ...
        if hasattr(controller, 'policy'):
            ...
            observations = []
            result = SimResult._make(
                (controller.states, observations, controller.actions, controller.steps, name))
        sim_results = []
        ...
        if param.env_name in ['SingleIntegrator', 'SingleIntegratorVelSensing', 'DoubleIntegrator']:
            ...
            for agent in env.agents:
                ...
                for k in np.arange(0, result.steps, 100):
                    X.append(
                        result.states[k, env.agent_idx_to_state_idx(agent.i)])
                    Y.append(
                        result.states[k, env.agent_idx_to_state_idx(agent.i)+1])
                    if param.env_name in ['SingleIntegrator', 'SingleIntegratorVelSensing']:
                        ...
                        U.append(
                            result.states[k, env.agent_idx_to_state_idx(agent.i)+2])
                        V.append(
                            result.states[k, env.agent_idx_to_state_idx(agent.i)+3])
                ...
                plotter.plot_square(
                    agent.s_g[0], agent.s_g[1], param.r_agent, angle=45, fig=fig, ax=ax, color=color)
            ...
            if param.env_name in ['SingleIntegrator']:
                for step in np.arange(0, result.steps, 1000):
                    ...
                    robot_pos = result.states[step, env.agent_idx_to_state_idx(
                        robot):env.agent_idx_to_state_idx(robot)+2]
                ...
                for step in np.arange(0, result.steps, 1000):
                    ...
                    for i in range(num_neighbors):
                        ...
                        ax.add_patch(
                            Circle(pos, param.r_agent, facecolor='gray', edgecolor='red', alpha=0.5))
                        idx += 4

                    for i in range(num_obstacles):
                        pos = observation[idx: idx+2] + \
                            robot_pos - np.array([0.5, 0.5])
                        ax.add_patch(
                            Rectangle(pos, 1.0, 1.0, facecolor='gray', edgecolor='red', alpha=0.5))
                        # pos = observation[idx : idx+2] + robot_pos
                    ...
                    # plot velocity vectors
                    ax.quiver(X, Y, U, V, angles='xy', scale_units='xy',
                              scale=0.5, color='red', width=0.005)
                ...
                for agent in env.agents:
                    for result in sim_results:
                        ax.plot(
                            ...
                            result.states[1:result.steps, env.agent_idx_to_state_idx(
                                agent.i)+i_config],
                            label=result.name)
                    ...
                    for result in sim_results:
                        ax.plot(
                            ...
                            result.actions[1:result.steps, agent.i *
                                           env.action_dim_per_agent+i_config],
                            label=result.name)
