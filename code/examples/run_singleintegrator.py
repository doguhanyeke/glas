...
class SingleIntegratorParam():
    def __init__(self):
        ...
        if self.safety == "cf_si_2":
            ...
            self.cbf_kp = 0.5
            self.epsilon = 0.01
            pi_max_thresh = self.kp / \
                (0.2 - self.r_agent) * 0.01  # 0.01 = epsilon
        ...
        # 'random','ring','centralplanner'
        self.il_load_dataset = ['orca', 'centralplanner']
        ...
        self.datadict = dict()
        # 100000000000 #10000000 #750000 #self.il_n_data
        self.datadict["8"] = 10000

        # learning hyperparameters
        # state dim, action dim, hidden layer, output phi, output rho
        n, m, h, l, p = 2, 2, 64, 16, 16
...
def run_batch(param, env, instance, controllers):
    ...
    for name, controller in controllers.items():
        ...
        states, observations, actions, step = run_sim(
            param, env, controller, s0, name=instance)
        states_and_actions = np.zeros(
            (step, 4*param.n_agents), dtype=np.float32)
        states_and_actions[:, 0::4] = states[:step, 0::env.state_dim_per_agent]
        states_and_actions[:, 1::4] = states[:step, 1::env.state_dim_per_agent]
        states_and_actions[:, 2::4] = actions[:step,
                                              0::env.action_dim_per_agent]
        states_and_actions[:, 3::4] = actions[:step,
                                              1::env.action_dim_per_agent]
    ...
    if args.batch:
        if args.controller:
            ...
            for ctrl in args.controller:
                ...
                if kind == "EmptyAPF":
                    controllers[name] = Empty_Net_wAPF(
                        param, env, torch.load(path))
                elif kind == "torch":
                    controllers[name] = torch.load(path)
                elif kind == "apf":
                    controllers[name] = Empty_Net_wAPF(
                        param, env, GoToGoalPolicy(param, env))
                else:
